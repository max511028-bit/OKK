...
# ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ: живой кандидат, который САМ просит перезвонить
# позже ("нет, перезвоните мне попозже"), может случайно совпасть с этим
# паттерном (рассчитан на автоответчик/объявление сети) — тогда звонок
# завершится как status=voicemail вместо честного "просил перезвонить".
# Отдельная обработка такой просьбы как отдельного статуса с авто-
# перезвоном — самостоятельная задача (не входит в этот пакет фиксов),
# пока это осознанный компромисс.
