...
# Если LLM недавно не ответила вовремя — не пытаемся снова эту паузу
# (реальный случай: недоступная/перегруженная Ollama держала кандидата
# в тишине 10+ секунд посреди живого разговора — таймаут 10с был
# специально подобран "с запасом", но с запасом на связь, а не на
# терпение живого человека на линии). 5 минут — одной кампании обычно
# хватает пережить временный сбой, не долбясь в недоступный сервис на
# каждом непонятном ответе.
_LLM_DEAD_UNTIL = 0.0


_EVASIVE_PATTERN = re.compile(
    r"(представьтесь|кто звонит|кто это|откуда у вас мой номер|кто вам дал|"
    r"откуда такая информация|расскажите подробнее|можно подробнее|"
    r"немного контекста|нужно подумать|надо подумать|нужно посоветоваться|"
    r"затрудняюсь ответить|нужно немного времени|повторите ещё раз|"
    r"заново расскажите|давайте позже скажу|что скажешь|передать сообщение)"
)


def answer_is_evasive(step: dict, raw: str) -> bool:
    """Уклончивый ответ на ПРОСТОЙ вопрос — поведенческий маркер
    «мимикрирующего» робота-секретаря (тест 17.07: ассистенты Яндекса
    отвечают встречными вопросами «представьтесь», «откуда мой номер»,
    «нужно подумать» вместо да/нет/возраста). Один такой ответ может дать
    и живой человек — поэтому phone_call копит СЧЁТЧИК и зовёт LLM только
    после двух подряд."""
    if not raw:
        return False
    t = _norm(raw)
    expect = (step or {}).get("expect")
    # На вопрос возраста живой называет число — уклончивость без цифр
    has_number = bool(re.search(
        r"\d|двадцат|тридцат|сорок|пятьдес|шестьдес|семнадцат|восемнадцат|девятнадцат",
        t)) or parse_age_from_text(raw) is not None
    if expect == "age" and not has_number and _EVASIVE_PATTERN.search(t):
        return True
    if _EVASIVE_PATTERN.search(t) and len(t.split()) >= 3:
        return True
    return False
...
# Живой кандидат, который САМ просит перезвонить позже ("нет, перезвоните
# мне попозже"), раньше мог случайно совпасть с _VOICEMAIL_PATTERN ниже
# (тот рассчитан на автоответчик/объявление сети) — звонок ошибочно
# завершался как status=voicemail вместо честного "просил перезвонить",
# и целевой кандидат терялся из воронки. is_callback_request() проверяется
# ПЕРЕД is_voicemail_phrase() везде, где идёт эта проверка по репликам —
# см. _run_dialog_loop в phone_call.py.
#
# Различаем по маркерам: корень "перезвон/позвон/наберите" + ЛИЧНЫЙ
# маркер (просьба от первого лица, отсылка к своему состоянию/времени) —
# и ОБЯЗАТЕЛЬНО отсутствие операторских маркеров (абонент/аппарат/номер/
# сеть/сигнал/сообщение/автоответчик — это уже точно не живой человек).
_CALLBACK_PERSONAL_MARKER = re.compile(
    r"(мне|я\b|можешь|можете|давай|сейчас не могу|не могу говорить|"
    r"занят\b|занята\b|за рулем|за рулём|минут|часик|\bчас\b|вечером|утром|"
    r"завтра|попозже)"
)
_CALLBACK_OPERATOR_MARKER = re.compile(
    r"(абонент|аппарат|номер|сеть|сигнал|сообщение|автоответчик)"
)
_CALLBACK_ROOT = re.compile(r"(перезвон|позвон.*позж|наберите|набери)")


def is_callback_request(raw: str) -> bool:
    """Похоже ли на просьбу ЖИВОГО кандидата перезвонить позже (а не на
    автоответчик/объявление сети, которые тоже часто говорят
    "перезвоните позже" — см. комментарий выше)."""
    if not raw:
        return False
    text = _norm(raw)
    if not _CALLBACK_ROOT.search(text):
        return False
    if _CALLBACK_OPERATOR_MARKER.search(text):
        return False
    if _CALLBACK_PERSONAL_MARKER.search(text):
        return True
    # 26.07: раньше требовался ТОЛЬКО личный маркер («мне», «я», «занят»…),
    # и самые естественные формулировки живых людей терялись: «перезвоните
    # позже», «наберите позже», «сейчас неудобно» → False, кандидат уезжал
    # в «не распознали»/спорные. Различитель, который остаётся безопасным:
    # ДЛИНА. Живой просит коротко (2-4 слова), а объявление оператора —
    # длинное и почти всегда содержит слова из _CALLBACK_OPERATOR_MARKER
    # (они отсеяны выше). Поэтому короткую просьбу принимаем.
    return len(text.split()) <= 4


CALLBACK_BYE_TEXT = "Хорошо, перезвоню вам позже. Хорошего дня!"

# Короткие подтверждения между вопросами (опция сценария "fillers", см.
# Часть 2 доработок 2026-07 — "человечность" бота: живые операторы почти
# всегда что-то бормочут между вопросами, а бот, который молча стреляет
# вопрос за вопросом, звучит заметно роботизированнее. Используются
# ТОЛЬКО перед НОВЫМ вопросом (не перед повтором/переспросом — там
# подтверждать нечего, ответ ведь не поняли), см. _run_dialog_loop.
FILLER_PHRASES = ["Ага.", "Угу.", "Так, записала.", "Поняла.", "Хорошо."]
...
_VOICEMAIL_PATTERN = re.compile(
    ...
    # Ещё формулировки основных операторов (МТС/Билайн/Мегафон/Т2),
    # накопленные по итогам реальных тестовых обзвонов — пополняется по
    # мере встречи новых вариантов.
    r"аппарат абонента выключен|аппарат выключен|"
    r"телефон абонента выключен|телефон выключен|"
    r"вне зоны обслуживания сети|вне зоны обслуживания|"
    r"невозможно установить соединение|невозможно соединить|"
    r"все каналы связи заняты|линия перегружена|сеть перегружена|"
    r"попробуйте позвонить\s+\S*\s*позже|попробуйте позвонить позднее|"
    r"вы можете перезвонить позднее|перезвоните позднее|"
    r"оставьте голосовое сообщение|запишите\s+(свое|своё)?\s*сообщение|"
    r"нажмите решетку|нажмите решётку|нажмите звездочку|нажмите звёздочку|"
    r"набранный вами номер не существует|такого номера не существует|"
    r"недоступен или находится вне зоны|"
    r"вызываемый абонент временно недоступен|"
    r"здравствуйте вы позвонили|"
    # Формулировки, реально прилетевшие на тесте 2026-07-10 (Яндекс-лавка),
    # которых не хватало (Эльмира/Елизавета уходили в «не распознано»):
    r"оставить сообщение|можете оставить сообщение|"
    r"после звукового сигнала|"
    r"перенаправлен на голосов|перенаправлен на голосовую|"
    r"телефон разряжен|разряжен или он находится|"
    # ВНИМАНИЕ: НЕ добавлять сюда «оставайтесь на линии» / «продолжаем
    # дозваниваться» — это ГУДОК ОЖИДАНИЯ Яндекса, а не голосовая почта.
    # (2026-07-10 из-за него Леонел, сказавший «алло да» после гудка, и
    # Константин ложно ушли в «автоответчик».) Настоящие заглушки, где
    # эта фраза встречается, и так ловятся по «не берет трубку»/«перезвоните
    # позднее»/«оставьте сообщение».
    # AI-голосовые ассистенты / нейросекретари (Яндекс и аналоги) — НОВЫЙ
    # класс заглушки: не «абонент недоступен», а робот, который ВЕДЁТ диалог
    # («я голосовой помощник, передам сообщение»). На тесте 2026-07-10
    # Нургул и Дарья — такие ассистенты, но были засчитаны как «годен».
    # Фразы специфичны для робота-секретаря, живой кандидат так не говорит.
    r"голосовой ассистент|голосовой помощник|виртуальн\w+ помощник\w*|"
    r"умный помощник|я не просто бот|"
    r"отвечаю на звонки|на звонки отвечаю|отвечать на звонки когда|"
    r"помогаю абоненту|помогать абоненту|по просьбе абонента|"
    r"передам\s+\S*\s*сообщение|я передам|передам ему|передам абоненту|передам все что|"
    ...
    # Тест 17.07: «мимикрирующие» секретари Яндекса не представляются
    # роботами, но проговариваются характерными оборотами:
    r"что я могу передать|могу передать для него|могу передать для неё|"
    r"умные алгоритмы|мои алгоритмы|данные аналитики|"
    r"как абонент может связаться|абонент может связаться с вами|"
    # Тест 21.07 (запись session_394102934): Яндекс-ассистент владельца —
    # «записала ваш ответ для абонента, пока что он занят другим звонком,
    # необходимо ли вам перезвонить». Реалтайм смял его реплику в «нет» →
    # ложный стоп «не ищет работу» с первой же фразы.
    r"запис\w+ ваш\w* ответ|занят други\w* звонком|"
    r"необходимо ли вам перезвонить|"
    # Тест 23.07 (кампания «Лавка НДЗ», контакт Ева): ассистент новой
    # формулировки — «какие контактные данные могу передать абоненту,
    # говорите, я всё запишу, мне нравится анализировать человеческую речь,
    # абонент, которого я записываю звонки». Анти-мимикрия (нет возраста)
    # его пометила ⚠, но в автоответчики он не ушёл — фразы не совпадали.
    r"контактные данные могу передать|данные могу передать абонент|"
    r"я вс[её] запишу|записываю звонки|анализировать человеческую речь)"
)
...
# Голосовой РИНГ-БЭК оператора (звучит, ПОКА идёт дозвон до абонента) —
# это НЕ автоответчик и НЕ кандидат: человек может взять трубку сразу
# после. Отдельно от _VOICEMAIL_PATTERN, потому что исход другой: не
# «автоответчик», а «не взял трубку» (перезвон уместен). Реальный тест
# Яндекс-3: «продолжаем дозваниваться до абонента, оставайтесь на линии»
# улетало в «не распознали». См. предупреждение у _VOICEMAIL_PATTERN.
_RINGBACK_PATTERN = re.compile(
    r"(продолжа\w* дозванива|продолжив дозванива|идет дозвон|идёт дозвон|"
    r"оставайтесь на линии|ожидайте ответа|ожидайте соединения|"
    r"дозваниваемся до абонента|дозваниваемся до абонент)"
)


def is_ringback_phrase(raw: str) -> bool:
    """Голосовой ринг-бэк «идёт дозвон, оставайтесь на линии» — не
    автоответчик; человек ещё может ответить. Исход — «не взял трубку»."""
    if not raw:
        return False
    return bool(_RINGBACK_PATTERN.search(_norm(raw)))
...
def all_reask_texts() -> list:
    ...
    texts.append(CALLBACK_BYE_TEXT)
    return texts
...
# Обиходное обрамление возраста для шага expect="age" (26.07). Без «мне»
# фраза «мне двадцать пять» декодировалась как «НЕ двадцать пять».
_AGE_FRAMING = [
    "мне", "мои", "лет", "год", "года", "полных", "около", "примерно",
    "уже", "почти", "только", "исполнилось", "будет", "сейчас",
]
# Минимальный набор «просит перезвонить» — единственное, что реально нужно
# ловить на предметных шагах помимо самого ответа (is_callback_request).
_CALLBACK_MIN = ["перезвоните", "позже", "потом", "не сейчас", "неудобно", "занят"]
...
def vocab_for_step(step: dict) -> Optional[list]:
    ...
    if expect == "age":
        # 26.07: НЕ подмешиваем сюда _YESNO_VOCAB (было). Разбор ответа на
        # этом шаге идёт ТОЛЬКО через parse_age_from_text — 50 слов-согласий
        # для него бесполезны, но конкурируют с числами при декодировании и
        # реально побеждали их на живых звонках: в карточках вместо возраста
        # оказывалось «конечно можно тринадцать ок думаю», «хорошо», «окей»,
        # «договорились» (все — из да/нет-словаря). Замер на шумной линии:
        # разбор возраста 67% → 83%. Плюс без «мне» фраза «мне двадцать пять»
        # распознавалась как «НЕ двадцать пять» — слова «мне» в словаре не
        # было, а «не» было. Компактный callback-набор оставляем, чтобы не
        # потерять живого, который на этом шаге просит перезвонить.
        return _NUMS + _AGE_FRAMING + _CALLBACK_MIN + ["[unk]"]
    if expect == "gender_male":
        return [
