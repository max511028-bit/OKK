...
async function _loadModels(){{
  ...
  try{{
    ...
    const r=await fetch(url+'/api/tags',{{headers:{{'ngrok-skip-browser-warning':'true'}}}});
...
async function _ask(msg){{
  ...
  try{{
    ...
    const url=await _resolveUrl();
    if(!url)throw new Error('AI оффлайн');
    const res=await fetch(url+'/api/chat',{{method:'POST',headers:{{'Content-Type':'application/json','ngrok-skip-browser-warning':'true'}},body:JSON.stringify(payload)}});
