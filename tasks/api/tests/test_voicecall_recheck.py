...
class TestAssistantPhrases2307:
    """Регресс с записи 23.07 (кампания «Лавка НДЗ», Ева): ассистент новой
    формулировки прошёл как «годен ⚠» — теперь должен уходить в автоответчик."""

    REAL = ("какие контактные данные могу передать абоненту говорите пожалуйста "
            "а я все запишу мне нравится слушать и анализировать человеческую речь "
            "абонент которого я записываю звонки для него")

    def test_real_transcript_caught(self):
        assert dialog.is_voicemail_phrase(self.REAL)

    def test_fragments_caught(self):
        for p in ["какие контактные данные могу передать абоненту", "а я всё запишу",
                   "я записываю звонки", "анализировать человеческую речь"]:
            assert dialog.is_voicemail_phrase(p), p

    def test_live_answers_still_clean(self):
        for p in ["да я записал ваш номер спасибо", "мне нравится ваша вакансия расскажите",
                   "да могу передать резюме на почту", "да двадцать пять москва готов выйти",
                   "какие у вас есть вакансии расскажите подробнее"]:
            assert not dialog.is_voicemail_phrase(p), p
