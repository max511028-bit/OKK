...
class BidzaarSource(BaseSource):
    ...
    # Типы процедур (замер 04.08 при статусе 1 «идёт приём заявок»):
    #   тип 1 — 2822 шт, ЗАКУПКИ: «Текущий ремонт завода», «Доставка
    #           сотрудников склад МО Красногорск», «Обслуживание розничной
    #           сети магазинов» — то, где мы поставщик;
    #   тип 2 —  619 шт, ПРОДАЖА имущества: «РЕАЛИЗАЦИЯ неремонтопригодных
    #           запчастей», «продажа пластиковых отходов», «Реализация
    #           невостребованных ТМЦ». Компания тут продаёт неликвиды, а не
    #           покупает услуги. На выборке из 30 названий — 25 явных
    #           продаж против нуля у типов 1 и 3;
    #   тип 3 — 2045 шт, ЗАКУПКИ: работы, поставки, квалификационные отборы.
    #
    # Берём 1 и 3. Тип 2 был ошибочно добавлен и убран в тот же день: я
    # принял разницу в количестве за «пропущенные закупки», не посмотрев,
    # что это вообще за процедуры. Для тендер-радара продажа неликвидов —
    # чистый шум.
    #
    # Статусы 2 и 3 (14 тыс. и 227 тыс.) — завершённые процедуры, подать
    # заявку туда уже нельзя. Берутся только через настройку `statuses`.
