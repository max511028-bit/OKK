...
class B2BCenterSource(BaseSource):
    ...
    # ВЫКЛЮЧЕНА 04.08.2026. Площадка ответила на наши запросы прямым
    # текстом: «Превышен максимальный лимит скорости просмотра страниц.
    # Регламент площадки НЕ ДОПУСКАЕТ ИСПОЛЬЗОВАНИЕ БОТОВ. Хотите стать
    # партнёром? Обращайтесь!»
    #
    # Это не техническая преграда, которую надо обойти, а заявленное
    # правило площадки. Обходить его — подставлять компанию: у владельца
    # там рабочий аккаунт, и блокировка прилетит уже не роботу.
    #
    # Легальные пути открыть данные снова:
    #   • партнёрство / официальная выгрузка — площадка сама это предлагает;
    #   • подписки на новые закупки в личном кабинете (их штатная функция
    #     для поставщиков) и приём этих уведомлений на нашей стороне.
    # Пока ни одного из них нет — коннектор остаётся выключенным, а не
    # долбит площадку в надежде проскочить.
    enabled_by_default = False

    notes = (
        "ВЫКЛЮЧЕНА: регламент площадки не допускает ботов, о чём она "
        "сообщает прямым текстом при обходе. Нужен официальный доступ "
        "(партнёрство) или подписки из личного кабинета. Коммерческие "
        "закупки; цена и документация и так видны только участникам."
    )
    ...
    def fetch(
        ...
        if queries and pages_ok == 0:
            raise SourceUnavailable(
                ...
                f"результатов. Площадка отдаёт страницу «Превышен максимальный лимит "
                f"скорости просмотра. Регламент площадки не допускает использование "
                f"ботов». Нужен официальный доступ — партнёрство или подписки из "
                f"личного кабинета.")
